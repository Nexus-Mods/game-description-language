...
import { renderLifecycleTestFile } from './lifecycle-emit.js';
...
const renderValueNode = (v: ValueNode): string => {
  ...
  if (v.kind === 'hookRef') {
    // hookRefs only appear in discovery.version and events.did-deploy today.
    // If a context: binding or modType.path ever uses { hook: ... }, we want this to throw loudly.
    throw new Error(`hookRef value (${v.hookId}) is not allowed in this position — only discovery.version and events.did-deploy accept hook refs`);
  }
  // Branch value.
...
`;
  ...
  const info = JSON.stringify({
    id: extensionId(doc.game.id),
    name: doc.game.name,
    // Vortex reads `author` here to derive game.contributed and official-vs-
    // community status. "Nexus Mods"/"Black Tree Gaming Ltd." are treated as
    // official (no "Contributed by" tag); any other value shows as community.
    ...(doc.game.author && { author: doc.game.author }),
  ...
  // Always emit lifecycle tests: per-store setup-creates-expected-paths +
  // queryModPath assertions + did-deploy hook wiring check. Resolves expected
  // values at codegen time so the emitted test pins literal strings.
  const lifecycleFile = renderLifecycleTestFile(doc);
  files.push({ path: 'lifecycle.gen.ts', contents: lifecycleFile });
